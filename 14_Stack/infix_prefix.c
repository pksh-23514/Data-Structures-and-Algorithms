...
Description  : Convert the Infix Expression to Prefix Expression.
...
int Infix_Prefix_conversion (char *Infix_exp, char *Prefix_exp, Stack_t *stk)
{
	...
	int pos_i = 0, pos_p = 0;	//To track the index of the Infix and Prefix array element position.
	...
	{
		if ((ch >= '0') && (ch <= '9'))		//If the character is a Digit; append it to the Prefix expression.
		{
			Prefix_exp [pos_p++] = ch;
		}
		else if ((ch == '+') || (ch == '-') || (ch == '*') || (ch == '/') || (ch == '%') || (ch == '^'))	//If the character is an Operator.
		{
			...
			{
				Prefix_exp [pos_p++] = peek (stk);	//Append the Operator on the Stack to the Prefix expression.
				pop (stk);
			}
		...
		else if (ch == ')')		//If a Close bracket is encountered, push it on the Stack.
		{
			push (stk, (int) ch);
		}
		else if (ch == '(')		//If an Open bracket is encountered, append all the characters from the Stack until the Open bracket is reached.
		{
			while (peek (stk) != ')')
			{
				Prefix_exp [pos_p++] = peek (stk);	//To append the Prefix expression with the Top-most element on the Stack.
	...
	{
		while (peek (stk) != -1)	//Until the stack becomes empty, append all the characters to the Prefix expression.
		{
			Prefix_exp [pos_p++] = peek (stk);
			pop (stk);
		}

		Prefix_exp [pos_p] = '\0';	//Terminate the Prefix expression with '\0' to convert it to a string.
	}
